...
const int Mnum = 13;   //M 
const int Nnum = 14;   // N 
const double eps[3] = { 0.10, 0.20, 0.30};
const int service_time[3] = { 10, 8, 6 };
const int service_hours = 10;
const int K_number = 1; // |T_k|
const int KK_num = 3; //|K| 
const int P = 1000000;  
...
double random_uniform_distribution(double min, double max) {
	return min + (max - min) * rand() / (RAND_MAX + 1.0);    //RAND_MAXXX is 0x7FFF
}
...
IloEnv env;
//******************************Multi*************************
void Multi_solution(double solu[10], double f[Mnum], double h[Mnum], int c[Mnum][Nnum], double Q[Mnum], double D_mean[Nnum], double DL[Nnum], double DU[Nnum], double sumDL, int Tao_ij[KK_num][Mnum][Nnum])
{
	IloNumArray YYY(env, Mnum),II(env, Mnum), III(env, Mnum); 

	NumMatrix II_k(env, KK_num);
	for (int k = 0; k < KK_num; k++) {
		II_k[k] = IloNumArray(env, Mnum);
	}


	IloNumArray uu(env, Nnum), vv(env, Mnum);
	ofstream eout("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/eoutoutput.txt");
	eout << "Starting" << endl;
	try
	{
		
		int base_num = Nnum / KK_num;

		int remainder = Nnum % KK_num;

		//A
		vector<vector<int>> A(KK_num);
		...
		for (int k = 0; k < KK_num; k++) {
			...
			int current_num = base_num;

			if (k < remainder) {
				current_num++;
			}

			A[k].resize(current_num);
		...
		vector<int> Size_A_k(KK_num);
		for (int k = 0; k < KK_num; k++) {
			Size_A_k[k] = A[k].size();
			cout << "Size_A_k[k]："<< Size_A_k[k] << endl;
		}
		...
		for (int k = 0; k < KK_num;k++) {
			for (int s = 0; s < Size_A_k[k]; s++) {
				he[k] += (DU[A[k][s]] - DL[A[k][s]]) * (DU[A[k][s]] - DL[A[k][s]]);
			}
		}
		...
		for (int k = 0; k < KK_num;k++) {
			...
			TI[k] = sumDM + sqrt(-log(eps[k]) * he[k] / 2);  
			eout << "TI[" << k << " ] = " << TI[k] << endl << endl;
		}
		...
		NumVarMatrix I(env, KK_num);                   
		for (int k = 0; k < KK_num; k++)
		{
			I[k] = IloNumVarArray(env, Mnum, 0, P, ILOFLOAT);   
		}

		//IloNumVarArray I(env, Mnum, 0, P, ILOFLOAT);   //Ii 
		IloNumVarArray y(env, Mnum, 0.0, 1.0, ILOINT);      // YYYi 


		
		NumVarMatrix3 q(env, KK_num); //q_kiS
		for (int k = 0; k < KK_num; k++) {
			q[k] = NumVarMatrix(env, Mnum);
			for (i = 0; i < Mnum; i++)
			{
				q[k][i] = IloNumVarArray(env);
			}
		}
		...
		for (int k = 0; k < KK_num; k++) {
			for (int s = 0; s < Size_A_k[k]; s++) { 
				for (int i = 0; i < Mnum; i++) {
					q[k][i].add(IloNumVar(env, 0.0, P, ILOFLOAT)); 
					
					model.add(P * Tao_ij[k][i][A[k][s]] * x[i][A[k][s]] - q[k][i][s] >= 0);
					model.add(q[k][i][s] <= I[k][i]);
				}
				...
				TTi = TI[k] - S_no_j;
				double minbd = compare(TTi, DU[A[k][s]]);
		...
		do
		{
			...
			for (int k = 0; k < KK_num; k++) {
				...
				IloNumVarArray u(env, Size_A_k[k], 0, 1, ILOINT);
				...
				submodel.add(z - exp1 + TI[k] - exp2 >= 0);
				exp1.clear(); exp1.end();
				exp2.clear(); exp2.end();
				...
				for (int s = 0; s < Size_A_k[k]; s++)
				{
					for (int i = 0; i < Mnum; i++)
					{
						if (Tao_ij[k][i][A[k][s]] * XXXX[i][A[k][s]] > 0.5)
						{
							submodel.add(v[i] >= u[s]);
				...
				eout << endl << "---------------------------------------" << endl;
				eout << "Best objective value：";
				objyan[k] = subsolver.getObjValue();
				eout << " objyan=" << objyan[k] << "   if bigger than 0" << endl;
			...
			for (int k = 0; k < KK_num; k++) {
				if (objyan[k] < -0.10)
				{
					...
					for (int i = 0; i < Mnum; i++) {
						...
						model.add(q[k][i][q[k][i].getSize() - 1] <= I[k][i]);
					...
					for (int i = 0; i < Mnum; i++)
					{
						...
						for (int s = 0; s < Size_A_k[k]; s++)
						{
							X_is += P * Tao_ij[k][i][A[k][s]] * x[i][A[k][s]] * uu[k][s];
						}
						model.add(q[k][i][q[k][i].getSize() - 1] <= X_is);
		...
		ofstream xout("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/XIJ.txt");
		for (i = 0; i < Mnum; i++)
		{
			for (j = 0; j < Nnum; j++)
			{
			...
			xout << endl;//solu[1] += XXX[i][j] * YYY[i];
		}


		ofstream yout("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/YI.txt");
		for (i = 0; i < Mnum; i++)
		{
			yout << "   " << YYY[i] << "   ";
		}
		ofstream Iout("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/II.txt");
		for (i = 0; i < Mnum; i++)
		{
			Iout << "   " << II[i] << "   ";
		}
...
void Single_solution(double single_solu[10], double f[Mnum], double h[Mnum], int c[Mnum][Nnum], double Q[Mnum], double D_mean[Nnum], double DL[Nnum], double DU[Nnum], double sumDL, int Tao_ij[K_number][Mnum][Nnum]) //函数声明，就是具体的求解方法  
{
	IloNumArray YYY(env, Mnum), II(env, Mnum), III(env, Mnum); 
	...
	IloNumArray uu(env, Nnum), vv(env, Mnum);
	ofstream gout("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/goutoutput.txt");
	...
	try
	{
		...
		for (int k = 0; k < KK_num; k++) {
			Size_A_k[k] = A[k].size();
			cout << "Size_A_k[k] size：" << Size_A_k[k] << endl;
		}
		...
		IloNumVarArray y(env, Mnum, 0.0, 1.0, ILOBOOL);      
		...
		for (i = 0; i < Mnum; i++)
		{
			...
			gout << endl;//solu[1] += XXX[i][j] * YYY[i];
		}
		gout << "----------------------------" << endl;
		...
		do
		{
			...
			for (int k = 0; k < KK_num; k++) {
				...
				gout << "Best objective value：";
				objyan[k] = subsolver.getObjValue();
				gout << " objyan=" << objyan[k] << "   if bigger than 0" << endl;
				zout = subsolver.getValue(z);
				gout << "z=" << zout << endl;
				subsolver.clearModel();
				subsolver.clear();
		...
		ofstream xout("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/single_XIJ.txt");
		for (i = 0; i < Mnum; i++)
		{
			for (j = 0; j < Nnum; j++)
			{
		...
		ofstream yout("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/single_YI.txt");
		for (i = 0; i < Mnum; i++)
		{
			yout << "   " << YYY[i] << "   ";
		}
		ofstream Iout("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/single_II.txt");
		for (i = 0; i < Mnum; i++)
		{
			Iout << "   " << II[i] << "   ";
		}
...
int main(int argc, char** argv)
{
	...
	ofstream fout("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/foutPoutput.txt");
	ofstream result("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/resultoutput.txt");
	...
	ifstream Qfile("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/input/Qi.txt");
	...
	ifstream Ffile("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/input/Fi.txt");
	for (int i = 0; i < 13; i++)
	{
		Ffile >> F[i];
	}
	...
	fout << "H cost:" << endl;
	ifstream Hfile("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/input/Hi.txt");
	...
	fout << "D cost:" << endl;      
	ifstream Cfile("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/input/Dij.txt");
	for (int k = 0; k < 13; k++) {
		for (int j = 0; j < 14; j++) {
			Cfile >> D[k][j];
		}
	...
	int Tao_ij[KK_num][Mnum][Nnum];
	for (int s = 0; s < KK_num; s++) {
		for (int i = 0; i < Mnum; i++)
		{
			for (int j = 0; j < Nnum; j++)
			{
	...
	double Coverage_distan[KK_num];
	for (int s = 0; s < KK_num; s++) {
		Coverage_distan[s] = 0.0;
	}

	for (int s = 0; s < KK_num; s++) {
		Coverage_distan[s] = 60 * service_time[s];
	}

	for (int s = 0; s < KK_num; s++) {
		for (int k = 0; k < 13; k++) {
			for (int j = 0; j < 14; j++) {
				if (D[k][j] <= Coverage_distan[s]) {
					Tao_ij[s][k][j] = 1;
				}
				else {
					Tao_ij[s][k][j] = 0;
	...
	for (int k = 0; k < cycle; k++)
	{
		...
		ifstream DMfile("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/input/MeanDj.txt");
		...
		ofstream dddout("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/doutDemoutput.txt");
		//////////////////////////////// normal //////////////////////////////////////////////////
		dddout << " normal " << endl;
		...
		Multi_solution(solu, f, h, c, Q, DM, DL, DU, sumDL, Tao_ij);
		result << "*****Multi*****" << endl;
		...
		ifstream OPin("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/open.txt");
		
		ifstream Yin("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/YI.txt");
		int Multi_Yi[Mnum];
		for (i = 0; i < Mnum; i++)
		{
			Yin >> Multi_Yi[i];
		...
		ifstream Iin("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/II.txt");
		...
		ifstream xin("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/XIJ.txt");
		...
		Single_solution(single_solu, f, h, c, Q, DM, DL, DU, sumDL, Tao_ij);
		result << "*****Single result*****" << endl;
		...
		ifstream single_OPin("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/single_open.txt");
		single_OPin >> OpenF[k][1];

		ifstream single_Yin("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/single_YI.txt");
		double single_Yi[Mnum] = { 0.0 };
		for (i = 0; i < Mnum; i++)
		{
			single_Yin >> single_Yi[i];
		}
		ifstream single_Iin("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/single_II.txt");
		...
		single_Yin.close();
		single_Iin.close();
		ifstream single_xin("C:/Users/20148/Desktop/extended study2/CPLEX_programming1/output/single_XIJ.txt");
